...
class SwiftStringsGeneratorTests < Minitest::Test
	...
	def test_generate_generates_empty_enum_when_no_keys_or_values
		assert_true file_is_generated_by_keys_values("empty_enum.swift", [], [])
	end

	def test_generate_generates_many_cases_when_keys_and_values
		assert_true file_is_generated_by_keys_values("two_case_enum.swift", ["key1", "key2"], ["value1", "value2"])
	end

	def file_is_generated_by_keys_values(file_name, keys, values)
		generated_code = SwiftStringsGenerator.new.generate(keys, values).split("\n")
